...
def main_code():
	...
	if len(idxs) > 0:
		...
		# for i in idxs.flatten():
			# xmin = min(xmin, boxes[i][0])
			# xmax = max(xmax, boxes[i][2]+boxes[i][0])
			# if(abs(W - xmax) > abs(xmin)):
		...
		for i in idxs.flatten():
			...
			if(LABELS[classIDs[i]] == "person"):
				...
				if(boxes[i][0]<xmin):
					...
					wmin = w
					ymin = y
				if(boxes[i][2]+boxes[i][0] > xmax):
					xmax = boxes[i][2]+boxes[i][0]
					wmax = w
					ymax = y
				if W/2 > x and W/2 < x+w:
					ymiddle = y
		...
		if(detected):
			...
			if wmean > W - xmax or wmean > xmin or xmin < 0 or xmax > W:
				x = (W - wmean)/2
				y = ymiddle
			elif(W - xmax > xmin):
				x = xmax
			...
			for client in clients:
					...
					string = json.dumps({'x': float(x) / float(W), 'y': float(y) / float(H), 'h': float(h)/float(H)})
